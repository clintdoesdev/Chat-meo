...
const FlowNodeDataSchema = z.object({
  ...
  // Seconds, capped to MAX_NODE_DELAY_SECONDS in engine/executor.ts (120) — kept slightly
  ...
  // A base64 `data:` URI — sized generously above a 2MB source image (see MAX_REPLY_IMAGE_BYTES
  // in node-inspector.tsx) to account for base64's ~33% overhead plus the "data:image/...;base64,"
  // prefix, without leaving the cap effectively unbounded.
  imageDataUri: z.string().max(2_900_000).optional(),
