...
export function Toast({ message, tone = "ok" }: { message: string; tone?: "ok" | "bad" }) {
  return (
    <div
      role="status"
      className={`toast-in pointer-events-none fixed bottom-6 left-1/2 z-[110] -translate-x-1/2 rounded-full border px-4 py-2.5 text-[13px] font-medium text-text shadow-[0_16px_40px_-14px_rgba(0,0,0,.9)] ${
