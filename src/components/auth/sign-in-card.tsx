...
function authErrorMessage(code: string | null): string | null {
  ...
  if (code === "AccessDenied") {
    return "That sign-in was denied — Chatmeo is in closed beta, so only existing accounts can sign in right now.";
  }
      ...
      <div className="relative z-10 w-full max-w-[400px] rounded-[22px] border border-line-2 bg-card p-[34px_30px]">
        <div className="mb-6 text-center">
          ...
          <h2 className="mt-3 text-[22px] font-bold">
            {step === "twoFactor" || mode === "in" ? "Welcome back" : "Sign up is closed for now"}
          </h2>
          <p className="mt-1.5 text-[13px] text-muted">
            {step === "twoFactor"
              ? "One more step to confirm it's you."
              : mode === "in"
                ? "Meo missed you. Sign in to keep building."
                : "Chatmeo is in closed beta — new accounts aren't open yet."}
          </p>
        </div>
            ...
            {mode === "up" ? (
              <div className="flex flex-col items-center gap-4 py-2 text-center">
                <p className="text-[13px] leading-relaxed text-muted">
                  We&apos;re not letting new people in just yet — reach out if you&apos;d like early
                  access, and check back once the beta opens up.
                </p>
                <button
                  type="button"
                  onClick={() => setMode("in")}
                  data-fx-skip
                  className="w-full rounded-full bg-grad-orange py-[11px] text-sm font-semibold text-white shadow-[inset_0_1px_0_rgba(255,255,255,.3),0_8px_24px_-8px_rgba(255,92,22,.6)] transition hover:brightness-110"
                >
                  Sign in instead
                </button>
              </div>
